...
<head>
  <meta charset="utf-8">
  <title>Gym Cañones | Entradas por Mes</title>
  ...
  <!-- CSS específico de ESTA página (scope aislado) -->
  <link rel="stylesheet" href="../../css/income_over_month.css">
...
<div class="page-container sidebar-collapsed" id="navbarcollapse">
  ...
  <!-- === SCOPE === -->
  <div class="main-content ff-income-month">
    <div class="row">
    ...
    <div class="ff-section">
      <h3 class="ff-page-title">Entradas por Mes</h3>
    ...
    <form class="ff-toolbar" onsubmit="showMember(); return false;">
      ...
      <select name="month" id="smonth" class="ff-input ff-input--select">
        ...
        <?php
          foreach ($formattedMonthArray as $num => $txt) {
            $selected = ($num == date('m')) ? 'selected' : '';
            echo '<option '.$selected.' value="'.$num.'">'.$txt.'</option>';
          }
        ?>
      </select>
...
<script>
function showMember(){
  var yearSel  = document.getElementById("syear");
  var monthSel = document.getElementById("smonth");
  ...
  xhr.open("GET","income_month.php?mm="+encodeURIComponent(month)+"&yy="+encodeURIComponent(year)+"&flag=0",true);
  xhr.send();
}
</script>
